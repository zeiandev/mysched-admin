...
export default function ViewGridClient({ initial }: { initial: Row[] }) {
  ...
  const byDay = useMemo(() => {
    ...
    Object.values(map).forEach(list => list.sort((a,b)=>toMin(a.start)-toMin(b.start)));
    return map;
  }, [initial]);
          ...
          <div key={d} className="relative grid" style={{ gridTemplateColumns: `120px 1fr` }}>
            ...
            <div className="relative border-b">
              ...
              <div className="relative" style={{ height: `${(endMin-startMin)}px` }}>
                {byDay[d].map(ev => {
                  const s = clamp(toMin(ev.start), startMin, endMin);
                  const e = clamp(toMin(ev.end),   startMin, endMin);
                  const top = s - startMin;
                  const height = Math.max(24, e - s);
                  return (
                    <div
                      key={ev.id}
