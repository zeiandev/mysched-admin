...
export default function ClassesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api<{ rows: Row[]; count: number }>(
        `/api/classes?${p.toString()}`
      )
      setRows(res.rows)
      setCount(res.count)
    } finally {
                      ...
                      <Td>
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            setRows((rs) =>
                              rs.map((x) =>
                                x.id === r.id ? { ...x, title: val } : x
                              )
                            )
                            update(r.id, { title: val }, () =>
                              setRows((rs) =>
                                rs.map((x) =>
                                  x.id === r.id ? { ...x, title: old } : x
                                )
                              )
                            )
                          }}
                      ...
                      <Td>
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            setRows((rs) =>
                              rs.map((x) =>
                                x.id === r.id ? { ...x, code: val } : x
                              )
                            )
                            update(r.id, { code: val }, () =>
                              setRows((rs) =>
                                rs.map((x) =>
                                  x.id === r.id ? { ...x, code: old } : x
                                )
                              )
                            )
                          }}
                      ...
                      <Td>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setRows((rs) =>
                              rs.map((x) =>
                                x.id === r.id ? { ...x, day: val } : x
                              )
                            )
                            update(r.id, { day: val }, () =>
                              setRows((rs) =>
                                rs.map((x) =>
                                  x.id === r.id ? { ...x, day: old } : x
                                )
                              )
                            )
                          }}
